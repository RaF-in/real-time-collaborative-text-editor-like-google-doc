package com.mmtext.editorserversnapshot.client.grpc;
...
public class ShareServiceClient {
    ...
    public ShareServiceClient(
        ...
        // Create stubs without setting deadlines at construction time
        // We'll set deadlines per-call to avoid stale deadline issues
        ...
        try {
            log.info("Testing initial connection to Share Service...");
            boolean healthy = checkServiceHealth();
            log.info("Share Service initial health check result: {}", healthy);
        } catch (Exception e) {
            log.warn("Share Service initial health check failed: {}", e.getMessage());
        }
    }
    ...
    public ShareServiceProto.CreateDocumentPermissionsResponse createDocumentPermissions(Document document) {
        ...
        try {
            ...
            // Create a fresh stub with a new deadline for each call to avoid deadline issues
            ShareServiceProto.CreateDocumentPermissionsResponse response =
                    shareServiceStub.withDeadlineAfter(5, TimeUnit.SECONDS)
                            .createDocumentPermissions(request);
            ...
            log.error("gRPC error creating permissions for document {}: {} - {} | Debug info: Channel state={}, Shutdown={}, Terminated={}",
                    documentId, code, description,
                    shareServiceChannel.getState(false),
                    shareServiceChannel.isShutdown(),
                    shareServiceChannel.isTerminated(), e);
    ...
    public ShareServiceProto.GetUserPermissionsResponse getUserPermissions(String documentId, String userId) {
        ...
        try {
            ...
            return shareServiceStub.withDeadlineAfter(5, TimeUnit.SECONDS)
    ...
    public boolean checkServiceHealth() {
        try {
            ShareServiceProto.HealthCheckResponse response =
                    shareServiceStub.withDeadlineAfter(3, TimeUnit.SECONDS)
